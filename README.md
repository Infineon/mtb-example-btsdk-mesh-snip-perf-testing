...
Mesh performance testing application can be used to evaluate the effect of ADV Tx Power, Packet Length, and Network Retransmission on the Reliability, Latency, Number of Hops, and Payload size as they relate to the Mesh performance.
...
A confirmation response is sent back to the Mesh Performance Tool. The response includes the original payload along with the TTL value at the time the message was received.  Mesh performance testing application then uses returned TTL value to estimate the number of hops it takes to complete the round-trip.
