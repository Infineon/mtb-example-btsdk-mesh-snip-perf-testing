# Mesh Performance Testing Application

## Overview
This application can be used to test mesh performance. It uses a customized version of Mesh vendor specific model available in the SDK. See the document "WICED-MeshClient-And-ClientControlMesh-User-Guide", section "Mesh Performance Testing".

Mesh Performance testing application can be used to evaluate the effect of ADV Tx Power, Packet Length, and Network Retransmission on the Reliability, Latency, Number of Hops, Payload size as they relate to the Mesh performance.

Mesh Performance Tool uses Vendor Data Set operations to configure above parameters on the Mesh nodes. Based on the opcode and the parameter values received from the performance tool, this application configures the level of ADV Tx Power[0-4] and the network retransmission of the node.

A confirmation response is sent back to the Mesh Performance Tool. The response includes the original payload along with the TTL value at the time the message was received.  Mesh Vendor Client application then uses returned TTL value to estimate the number of hops it takes to complete the round-trip.
...
2. Follow the "Testing Procedure" mentioned in the document "WICED-MeshClient-And-ClientControlMesh-User-Guide".

These flags in the application makefile control whether the prebuilt mesh libs (core, models, and provisioner) will have debug traces enabled.

MESH\_MODELS\_DEBUG\_TRACES ?= 1<br/>
MESH\_CORE\_DEBUG\_TRACES ?= 1<br/>
MESH\_PROVISIONER\_DEBUG\_TRACES ?= 1
